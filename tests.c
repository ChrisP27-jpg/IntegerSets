...
START_TEST(otherIntersection)
{
  const int elements1[] = { 1, 3, 33, 45, 2, 53, 53 };
  const int nElements1 = sizeof(elements1)/sizeof(elements1[0]);
  const int elements2[] = { 1, 33, 54, 3, 45,   };
  const int nElements2 = sizeof(elements2)/sizeof(elements2[0]);
  const int intersectionElements[] = { 1, 3, 33, 45, };
...
intersectionIntSetSuite(void)
{
  ...
  tcase_add_test(intersectionTests, otherIntersection);
